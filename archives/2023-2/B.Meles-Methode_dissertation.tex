...
\hypersetup{
 ...
 pdfkeywords={},
 pdfsubject={},
 pdfcreator={Emacs 26.3 (Org mode 9.5.5)}, 
 pdflang={French}}
\begin{document}
...
\begin{enumerate}
\item comment concevoir un plan détaillé au brouillon (section \ref{org9ae085d}) ;
\item la composition de l'introduction, du développement et de la conclusion
(sections \ref{org9b028eb} à \ref{org00c93b1}) ;
\item un exemple de plan détaillé et de dissertation rédigée
(sections \ref{org0471005} et \ref{orgeea9169}) ;
\item une liste de sujets de dissertation (section \ref{org2c72197}) ;
\item (à destination des enseignants de philosophie) quelques propositions
sur l'enseignement de la méthode de la dissertation.
\end{enumerate}
...
vue du sens commun (voir plus bas la section \ref{orgb57278b}) ;

\item \uline{construire la problématique}, qui justifie le traitement
philosophique du sujet (voir plus bas la section \ref{org6b2209e}).
...
(voir plus bas la section \ref{org2b7b9b6}) ;
...
Vous trouverez un exemple de plan détaillé dans la section \ref{org0471005}.
Le plan détaillé doit contenir : 

\begin{enumerate}
\item des \uline{grandes parties, dont chacune ait pour titre une thèse répondant
explicitement à la problématique} (voir la section \ref{orge68d8bd}) ;

\item des \uline{sous-parties, dont chacune ait pour titre une thèse contribuant
à démontrer la thèse de la partie} (voir la section \ref{org6e163ef}) ;
...
(voir la section \ref{org225a86b}).
\end{enumerate}


\chapter{Introduction et conclusion}
\label{sec:org651d11b}
...
sections \ref{org9b028eb} sur l'introduction et \ref{org00c93b1} sur
la conclusion.

Voir un exemple de dissertation rédigée dans la section \ref{orgeea9169}, que
vous pourrez comparer avec le plan détaillé de la section \ref{org0471005}.

\part{Les parties de la dissertation}
\label{sec:org9efab61}

\chapter{L'introduction}
\label{sec:orga6b53a7}
\label{org9b028eb}

L'introduction doit être la justification, progressive et convaincante,
de la problématique.
...
ces lignes : voir la section \ref{org2c4fc32}) ;

\item un paragraphe par \uline{\emph{définition} des termes du sujet} (voir la
section \ref{orgb57278b}) ;

\item un ou deux paragraphes de \emph{problématisation} montrant l'existence
d'une \uline{contradiction apparente à résoudre} (voir la
section \ref{org6b2209e}) ;
...
comme une interrogation totale, c'est-à-dire une question appelant une
réponse par oui ou non (pour en savoir plus, voir la section \ref{orgd90a539}).


\section{La problématique}
\label{sec:org9c6fcd2}
...
\label{sec:org7c35751}
\label{org2b7b9b6}

\section{Les parties}
\label{sec:orgfef2618}
\label{orge68d8bd}
...
sous-parties dans la section \ref{orgeea9169}).
...
\label{sec:org8852388}
\label{org2c72197}

\chapter{Liste de sujets de dissertation}
\label{sec:org6592043}
...
principale source de hors-sujet}.

\subsection{Typologie des questions}
\label{sec:orgf54d508}

Il existe deux types de questions, le second se subdivisant à son tour
en de nombreux sous-types :

\begin{enumerate}
\item les \emph{interrogations totales} sont celles qui portent sur
l'intégralité d'une proposition. Exemples : « Apprendre
s’apprend-il ? », « Dieu a-t-il pu vouloir le mal ? », « Avons-nous
une âme ? », « Doit-on respecter la nature ? », « Peut-on classer les
arts ? », etc. \uline{Toutes les questions, et elles seules, appellent une
réponse par oui ou non} ;

\item les \emph{interrogations partielles} sont celles qui portent uniquement
sur un élément de la proposition, qui, selon les cas, peut être :
\begin{itemize}
\item le sujet (exemple : « \emph{Qui} connaît le mieux mon corps ? ») ;
\item l'attribut du sujet (exemple : « \emph{Qu'est-ce que} le hasard ? ») ;
\item un complément d'objet direct (exemple : « \emph{Que}
désirons-nous ? ») ;
\item un complément d'objet indirect (exemple : « \emph{À qui} dois-je la
vérité ? ») ;
\item des compléments circonstanciels (exemples : « \emph{À quoi bon}
démontrer ? », « \emph{Où} est le passé ? », « \emph{Quand} faut-il désobéir
aux lois ? », « \emph{Comment} s’orienter dans la pensée ? »,
« \emph{Pourquoi} plusieurs sciences ? »).
\end{itemize}
...
\label{sec:orgddb2d78}

\uline{Le type de question détermine fortement les termes de la contradiction
apparente} que l'on doit mettre au jour dans les deux paragraphes de
problématisation, après les définitions des termes :

\begin{enumerate}
\item si le sujet est une \emph{question totale}, alors l'un des deux
...
dans la section \ref{orga56254e}) ;
...
En ce sens, vous devez \uline{identifier les « attendus » de la question}, et
les justifier à partir du sens commun. \uline{Ne dites pas surtout pas
« le sujet présuppose que\ldots{} »}, comme s'il s'agissait de présupposés
arbitraires : il s'agit généralement de présupposés communément admis,
que vous devez accepter comme points de départ.


\subsection{Conséquences sur la problématique}
\label{sec:org27a1f0f}
...
le sujet « Dieu a-t-il pu vouloir le mal ? » (question totale) ne doit
...
sujet est une question totale, la problématique doit être une question
totale. Si le sujet est une question partielle portant sur un complément
circonstanciel de lieu, il doit en aller de même de la problématique.
...
\label{sec:org4667cf7}

\uline{Les titres de parties doivent également respecter le type de la
question}.

\uline{Si le sujet est une \emph{question totale}, alors les titres de parties
