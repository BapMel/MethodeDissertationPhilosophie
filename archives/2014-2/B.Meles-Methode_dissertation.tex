...
\epigraph{La vraie éloquence se moque de l'éloquence.}{Blaise Pascal,
  \emph{Pensées}, Br.~4.}
...
Dans la méthode ci-dessous, nous décrirons la structure logique de la
dissertation. Si, comme nous essaierons de le montrer, chacune des
étapes de cet exercice possède un réel rôle argumentatif, la rhétorique
y deviendra inutile.
...
substance», «C) le primat logique de la substance», «II~- La substance
comme illusion»).
...
Lorsque le sujet contient plusieurs concepts («Bonheur et vertu», «Toute
...
Kant a très bien formulé les conditions d'une bonne définition~:
\begin{quote}
  ...
  \end{enumerate}
\end{quote}
Kant a même fourni une méthode pour dégager les définitions~:
\begin{quote}
...
\item \emph{deux concepts} (ou, plus rarement, trois)~: «Substance et
  ...
  inconscient», «Pensée et calcul», «Mathématiques et philosophie», 
  «Ordre, nombre, mesure», etc.
\item \emph{une question}~: «Toute philosophie est-elle systématique~?»,
  ...
  l'humanité comme l'histoire d'un homme~?», etc.
...
Lorsqu'un sujet comporte deux termes (très rarement trois, comme «Ordre,
...
hamburgers tranche par tranche~: par exemple, de traiter, pour «Genèse
et structure», d'abord la genèse, ensuite la structure, enfin les
relations entre elles. Dans un tel traitement, seule la troisième partie
serait dans le sujet. Il faut traiter d'entrée de jeu les relations
...
les plus faciles. C'est généralement la forme que prennent les sujets de
baccalauréat. 

\par

Il faut cependant bien prendre garde à deux pièges~:
\begin{itemize}
\item que la nécessité de poser la question ait bien été expliquée en
  introduction, car la question ne doit pas paraître arbitraire~;
...
Lorsque le sujet est une citation, celle-ci ne doit jamais être prise au
pied de la lettre. Au risque de paraître jouer sur les mots, les deux
sujets suivants appellent bel et bien des traitements distincts~:
\begin{itemize}
...
\item il est généralement le plan le plus attendu --- or ce qui ne
  ...
  même plan fade se voit reproduit en trente exemplaires (quand ce n'est
  pas deux cents pour des concours comme celui de l'École Normale
  Supérieure)~;
...
pertinents que tous les plans génériques --- dialectique, criticiste,
réhabilitation, dégradation --- dont vous aurez entendu parler. Ce plan
est, à chaque fois, à inventer pour la première fois. S'il demande de
l'audace, il est souvent bien plus payant que tous les autres types de
plans.


\subsection{Quelles thèses faut-il soutenir~?}

Les candidats doivent comprendre que, en dehors de cas exceptionnels et
évidents (comme l'assertion de thèses racistes ou négationnistes), ils
ne sont jamais jugés sur leurs idées.

\par

Le correcteur n'attend pas des copies la confirmation de ses propres
convictions philosophiques~: il veut lire des copies argumentées. Rien
n'est pire que de voir nos propres thèses être mal défendues. On préfère
largement une copie défendant bien une thèse ennemie à une copie
défendant mal une thèse amie~: la première sera philosophiquement
beaucoup plus stimulante.
...
ce cas, écartez-la \emph{explicitement}, pour prévenir tout malentendu
...
défavorables à sa démonstration\footnote{Gilles-Gaston Granger,
  ...
  Philosophies pour l'âge de la science, 1968, p.~7.}».
...
ont souvent un effet pervers~: pour rentabiliser l'effort qu'a nécessité
...
question» ou «la guerre est un objet complexe». On peut certes conclure
sur une impossibilité de trancher, mais elle doit être argumentée, et
non s'appuyer sur la seule diversité des opinions. La diversité des
opinions n'est plus un bon point d'arrivée de dissertation qu'un bon
point de départ.
...
procédé confère à la dissertation une efficacité qui n'est pas
simplement rhétorique, mais argumentative~: il montre que vous saviez
dès le départ où vous alliez, et que le cheminement n'a pas été
improvisé ligne après ligne.
...
\section{Annexe~: comment les correcteurs lisent les copies}

Savoir sur quels critères vous êtes évalué vous permettra de rédiger des
copies satisfaisant le mieux possibles les attentes du correcteur. Voici
un exemple de lecture de copie, qui ne prétend pas être universel.

\par

Le correcteur lit d'abord l'introduction. À~ce stade, il a souvent une
idée de la note à quatre points près. C'est comme s'il raisonnait par
grandes cases~:
\begin{itemize}
...
\item une case D pour les copies inachevées ou bâclées, en dessous
  de~6.
\end{itemize}
Après avoir lu l'introduction, le correcteur lit parfois directement la
conclusion. Généralement, la lecture du développement ne fera pas
changer la copie de case, ou du moins pas dans un sens favorable au
candidat, car une copie qui commence et qui finit mal contient rarement
...
pertinence des exemples et des références philosophiques. Le développement
...
\item Introduction~:
  \begin{itemize}
  \item Les principaux termes du sujet ont-ils été définis~?
...
\item Développement~:
  \begin{itemize}
  \item L'argumentation est-elle structurée~?
  \item Chaque thèse est-elle démontrée~?
  \item La réflexion est-elle illustrée par des exemples bien analysés~? 
...
Contrairement à ce que l'on pense parfois de la dissertation, ces
attentes n'ont rien de spécifiquement rhétorique~: ce sont les qualités
que l'on est naturellement en droit d'attendre d'une argumentation en
général. L'honnêteté argumentative doit vous tenir lieu de rhétorique,
ce qui vous permettra de donner à vos options philosophiques la force
qu'elles méritent.
